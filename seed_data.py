...
def seed():
    ...
    # ---------- USERS ----------
    admin = User(username='admin', email='admin@college.tz', full_name='Head Admin',
                 role='admin', campus_id=None)
    admin.set_password('admin123')

    qa_head = User(username='qahead', email='qahead@college.tz', full_name='QA Head Office',
                   role='qa_officer', campus_id=head.id)
    qa_head.set_password('qa123')

    admin1 = User(username='admin1', email='admin1@dar.tz', full_name='Campus Admin Dar',
                  role='admin', campus_id=campus1.id)
    admin1.set_password('admin123')

    qa1 = User(username='qadar', email='qadar@dar.tz', full_name='QA Dar',
               role='qa_officer', campus_id=campus1.id)
    qa1.set_password('qa123')

    lecturer1 = User(username='jmushi', email='jmushi@dar.tz', full_name='Dr. John Mushi',
                     role='lecturer', campus_id=campus1.id)
    lecturer1.set_password('lecturer123')

    student1 = User(username='student1', email='student1@dar.tz', full_name='Amina Juma',
                    role='student', campus_id=campus1.id)
    student1.set_password('student123')

    student2 = User(username='student2', email='student2@dar.tz', full_name='Baraka Mwanga',
                    role='student', campus_id=campus1.id)
    student2.set_password('student123')

    director1 = User(username='director1', email='director1@dar.tz',
                     full_name='Dr. Sarah Mwamba', role='director',
                     title='Dr.', campus_id=campus1.id)
    director1.set_password('director123')

    ceo = User(username='ceo', email='ceo@college.tz',
               full_name='Prof. Ali Hassan', role='ceo',
               title='Prof.', campus_id=None)
    ...
    # ---------- FORM ----------
    form = Form(title='Course Evaluation Form (TPSC)', description='Standard evaluation form',
                created_by=qa_head.id, is_active=True)
    db.session.add(form)
    db.session.commit()

    def add_question(form_id, section_id, text, qtype='likert', required=True,
                     order=1, options=None, parent_question_id=None, condition_value=None):
        q = FormQuestion(
            form_id=form_id,
    ...
    add_question(form.id, sec1.id,
                 'Are the contents of this course adequate to give you required competence?', order=2)

    sec2 = FormSection(form_id=form.id,
                       title='SECTION 2: Please rate the Facilitator PUT NAME OF LECTURE on the following areas.',
                       order=2)
    db.session.add(sec2)
    db.session.flush()
    sec2_questions = [
    ...
    db.session.add(sec6)
    db.session.flush()
    add_question(form.id, sec6.id,
                 'Is there unethical/inappropriate behavior observed from this facilitator?',
                 qtype='multiple_choice', options=['Yes', 'No'], order=1)
    add_question(form.id, sec6.id, 'What good can you tell about this facilitator?',
                 qtype='text', required=False, order=2)
    q6_3 = add_question(form.id, sec6.id,
                        'Would you suggest that this facilitator continue to teach this subject?',
                        qtype='multiple_choice', options=['Yes', 'No'], order=3)
    db.session.flush()
    add_question(form.id, sec6.id, 'If answer is "No", please explain why.',
                 qtype='text', required=False, order=4,
                 parent_question_id=q6_3.id, condition_value='No')
    add_question(form.id, sec6.id,
                 'Your suggestions for further improvement of learning at TPSC',
                 qtype='text', required=False, order=5)

    sec7 = FormSection(form_id=form.id, title='SECTION 7: Learning environment and facilities', order=7)
    db.session.add(sec7)
    db.session.flush()
    for i, text in enumerate(['Toilets', 'Classes', 'College Compounds', 'Current Reading materials',
                               'Furniture (e.g chairs, tables etc)', 'Computers'], start=1):
        add_question(form.id, sec7.id, text, order=i)

    db.session.commit()

    # ---------- COURSES (after form is created) ----------
    c1 = Course(code='PA101', name='Intro to Public Admin', campus_id=campus1.id,
                course_type='long', department_id=dept1.id, programme_id=prog1.id,
                level='NTA Level 5', semester='Semester 1', academic_year='2025/2026',
                lecturer_id=lecturer1.id, form_id=form.id)
    c2 = Course(code='IT101', name='Computer Fundamentals', campus_id=campus1.id,
                course_type='long', department_id=dept2.id, programme_id=prog2.id,
                level='NTA Level 5', semester='Semester 1', academic_year='2025/2026',
                lecturer_id=lecturer1.id, form_id=form.id)
    import uuid
    c3 = Course(code='SHRT1', name='Effective Leadership', campus_id=campus1.id,
                course_type='short', department_id=dept1.id, programme_id=prog3.id,
                lecturer_id=lecturer1.id, form_id=form.id,
